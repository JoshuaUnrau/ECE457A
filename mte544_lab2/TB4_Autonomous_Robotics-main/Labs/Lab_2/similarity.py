...
class similarity():
    ...
    def filter_reader(self, points, range):
        filtered_points = []
        for point in points:
            if self.distance(0, 0, point[0], point[1]) < range:
                filtered_points.append(point)
        return filtered_points

    #Robot pos (x,y, rot)
    def similarity_check(self, point_toogle: int, simulated_points, robot_pos, do_plot, selection):
        self.point_3_data_filtered = point_3_data_all[selection]
        self.point_6_data_filtered = point_6_data_all[selection]
        ...
        rotated_lidar_points = self.filter_reader(rotated_lidar_points, 100)
